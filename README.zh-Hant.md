...
| 冰箱：`Panasonic-NR-*` → `192.168.102.1`，或 BLE＋QR | **僅雲端** | **不能**靠 57223；BLE 見下方評估 |

官方 App **日常遙控不走 SetSaanet**；本套件是同一硬體埠上的**平行本地協定**。

### BLE 評估（依「非配對、要有可用讀值」條件 → **不做**）

官方 App 的 BLE 全部在 **`OnboardingManager`**（配對套件）裡；冰箱主畫面等日常 UI **零引用** BLE。

GATT 實際指令只有這些：

| BLE／SoftAP 命令 | 用途 |
| --- | --- |
| `GetProfile`／`GetMacAddr`／`GetStatus` | 模組身分／配對狀態（非冷氣溫度、非冰箱門） |
| `GetSsidList`／`GetSecurityType`／`SsidPskSetup`／`STAConnect` | 掃 Wi‑Fi、寫入家用帳密 |
| `Regist`／`StartRegMode`／`StopRegMode` | 雲端註冊／結束註冊模式 |

**結論：** 去掉配對之後，BLE **沒有任何對 HA 有價值的可讀／可寫狀態**（沒有電源、溫度、模式、門開等）。因此本整合**不實作 BLE**。無 `57223` 的設備請用雲端整合；若未來要控冰箱，應另開雲端專案。
