...
APK 的 BLE 主要用於**配對／寫入家用 Wi‑Fi**（GATT 命令近似 SoftAP 的 GetProfile／Regist），**不是**配對完成後的日常遙控通道。

| 若把 BLE 加進 HA | 結果 |
| --- | --- |
| 冷氣／除濕 | 配對後本地控仍靠 **57223**；BLE 幫不了「找不到埠」 |
| 冰箱 | 配對後 App 也改走**雲端**；BLE 視窗短暫，無法取代日用控制 |
| 實作成本 | HA 主機需藍牙適配器、近距、逆向 GATT、維護成本高 |

**結論：本整合不納入 BLE。** 冰箱／無 57223 設備請繼續用官方 App 或雲端型 HA 整合（如 `panasonic_smart_app`）。若未來要做「雲端冰箱」，應是**另一套整合**，不是擴充本套件的 SetSaanet 路徑。
